...
const GRUPLAR: PresetGroup[] = [
  {
    ...
    color: '#2E7D5B',
    services: [
      { name: 'Hydrafacial', duration_minutes: 60, color: '#2E7D5B', common: true },
      { name: 'Karbon peeling', duration_minutes: 45, color: '#2E7D5B', common: false },
    ],
  },
  {
    name: 'Medikal estetik',
    color: '#5B4FA3',
    services: [{ name: 'Botoks', duration_minutes: 30, color: '#5B4FA3', common: true }],
  },
];
...
describe('HizmetSecimi', () => {
  it('hizmet seçince süresini ve rengini de taşır', () => {
    ...
    expect(secilenler()).toBe('Botoks|30|#5B4FA3');
  });

  it('seçili hizmete tekrar basınca kaldırır', () => {
    render(<Kabuk baslangic={[{ name: 'Botoks', duration_minutes: 30, color: '#5B4FA3' }]} />);
    fireEvent.click(screen.getByText('Botoks'));
  ...
  it('grubun tamamını tek tıkla seçer ve kaldırır', () => {
    ...
    expect(secilenler()).toBe('Hydrafacial|60|#2E7D5B,Karbon peeling|45|#2E7D5B');
  ...
  it('merkezin kendi yazdığı hizmeti listeye ekler', () => {
    ...
    expect(secilenler()).toBe('Özel bakımım|60|#2E7D5B');
  });
